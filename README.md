...
| **Mount as a drive letter or folder** | Through [WinFsp](https://winfsp.dev/rel/) (free user-mode file-system framework) the volume is exposed **read-only as a normal Windows drive** (e.g. `R:`, registered with the mount manager so it is visible to every program) or inside a folder you pick, so Explorer and any program can open files from it. Windows' own NTFS driver and partition manager are bypassed entirely, which also works for disks Windows refuses to show at all. |
...
| **Repair** | Restore NTFS boot sector from its backup, rewrite the backup boot sector, rebuild the primary GPT from the backup GPT, restore `$MFT` records 0–3 from `$MFTMirr`. Each write is preceded by a JSON backup of the replaced sectors; **Undo** restores them. Manual guidance (bcdboot/bootrec) is generated for boot-manager problems. Dynamic (LDM) disks whose simple volume fills its partition can be converted to basic in place by retyping the partition entry; the NTFS is untouched. |
| **Windows tools** | Windows' own view of the storage stack (Get-PhysicalDisk / Get-Disk / Get-Volume / Get-Partition / Get-PnpDevice / services / event log / diskpart list disk & list volume) with findings that compare the layers and one-click fixes that show the exact command: online/offline, clear read-only, import foreign dynamic disk, assign/remove letter, re-detect device, remove phantom entries, automount, restart storage services, rescan, read-only chkdsk. Console and TXT/HTML/PDF report. |
...
nova4me2 mount 2 R:                             read-only drive letter via WinFsp (Ctrl+C unmounts); a folder path works too
...
nova4me2 forensics D:\\drive.img badsectors                             which files the unreadable sectors of an image belong to
nova4me2 windows status                                                   what Windows sees (disks, volumes, findings)
nova4me2 windows import 0 | online 0 | assign 5 T | rescan | events --hours 24 | report win.html
```
